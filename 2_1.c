...
int power_4 (int number, int pow); // Task 5.4)
double power_5 (double number, int pow); // Task 5.5)
int power_6 (int number, int pow); // Task 5.6) System call_1
int power_7 (int number, int pow, int result_); // Task 5.7) System call_2
...
int main ()
{
    ...
    // 5.5) 
    printf("5.5) %.0lf\n", power_5 (number_a, power));

    // 5.6) System call_1
    printf("5.6) system call_1: %.0lf\n", power_6 (number_a, power));

    // 5.7) System call_2
    int result_ = 1;
    printf("5.7) system call_2: %d\n", power_7 (number_a, power, result_));
...
double power_5 (double number, int pow)
{
    if (pow == 0) return 1;
    else if (pow <= 0) return power_5 (1 / number, -pow);

    int lg = (int) log2f(pow);
    double result = 1, pows[lg + 1];
    pows[0] = number;
    ...
    for (int i = 0; i <= (int) log2f(pow); i++) {
        if (pow & (1 << i)) result *= pows[i];
    }

    return result;
}

int power_6 (int number, int pow)
{
    if (pow == 0) return 1;
	if (pow == 1) return number;

	return power_6(number, pow / 2) * power_6(number, pow / 2) * power_6(number, pow % 2);
}

int power_7 (int number, int pow, int result_)
{
    if (pow >= 1)
	{
		result_ *= number;
		pow--;
		power_7(number, pow, result_);
	}
	else
	{
		return result_;
