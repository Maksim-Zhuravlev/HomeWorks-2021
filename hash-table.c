...
double av_len_list(struct HashTable *table) {
    size_t n = 0;
    size_t len = 0;
    for(size_t i = 0; i < table -> size; i++) {
        struct LinkedList *bucket = &table -> buckets[i];
...
size_t len_min(struct HashTable *table) {
...
size_t len_max(struct HashTable *table) {
    size_t max_len = 1;
    for(size_t i = 0; i < table -> size; i++) {
        struct LinkedList *bucket = &table -> buckets[i];
        if (bucket -> size != 0) {
...
int main() {
    ...
    for(int i = 1; i <= 100000; i *= 10) {
        ...
        file = fopen("geroi_nashego_vremeni.txt", "r");
        if (file == NULL) {
