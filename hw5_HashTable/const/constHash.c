...
int notNullLists(struct HashTable* table) {
    int n = 0;
    for (size_t i = 0; i < table->size; i++) {
        struct LinkedList* bucket = &table->buckets[i];
        if (bucket->size != 0) {
...
double listLen(struct HashTable* table) {
    size_t n = 0;
    size_t len = 0;
    for (size_t i = 0; i < table->size; i++) {
        struct LinkedList* bucket = &table->buckets[i];
...
size_t minLen(struct HashTable* table) {
...
size_t maxLen(struct HashTable* table) {
    size_t max_len = 1;
    for (size_t i = 0; i < table->size; i++) {
        struct LinkedList* bucket = &table->buckets[i];
        if (bucket->size != 0) {
...
void normalWords(char* word, char* normal_word) {
    keyType not_normal = "!?.,():;/";
    int g = 0;
    int i = 0;
    while (word[i] != '\0') {
