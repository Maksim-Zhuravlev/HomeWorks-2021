...
int main()
{
	...
	int j = 0;
	int n = strlen(s);
	for (int i = 0; i < n; i++) // организуем цикл для посимвольного чтения s (от 0 элемента до последнего)
	{
	...
	for (int i = 0; i < y; i++)
	{
		if (t[i] != t[y - (i + 1)])  // strlen(t)-(i+1) - индекс массива зеркальный относительно элемента t[i]
		{
			result = 0;
			break;
		}
